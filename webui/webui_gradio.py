...
DEFAULT_SYSTEM_PROMPT = """You are a helpful, respectful and honest assistant.Help humman as much as you can."""
...
parser.add_argument('--port', default=7777, type=int, help='Port of gradio demo')
parser.add_argument(
    '--max_memory',
    default=1024,
    type=int,
...
#<img src={github_banner_path}  width="60" height="20" />
...
with gr.Blocks(css=customCSS, theme=small_and_beautiful_theme) as demo:
    gr.Markdown("""<h1><center>Chatbot</center></h1>
        ...
        """)
    # gr.Markdown("# <center> 🦙💬 MosesLM-Chatbot 🚀 </center>")
    # github_banner_path = 'https://raw.githubusercontent.com/moseshu/llama2-chat/main/llama2.jpg'
    # gr.HTML(f'<p align="center" style="font-size: 48px"><a href="https://huggingface.co/Moses25/MosesLM-13B-chat" >🦙💬 Moses-Chatbot 🚀 </a></p>')
    state = gr.State()
    ...
    with gr.Row():
        with gr.Column(scale=1):
            system_prompt_input = gr.Textbox(
                ...
                lines=1)#.style(container=True)
            negative_prompt_input = gr.Textbox(
                show_label=True,
                label="反向提示语（仅在对话开始前或清空历史后修改有效，对话过程中修改无效）",
                placeholder="option",
                lines=1,
                visible=ENABLE_CFG_SAMPLING)#.scale(container=True)
        
        with gr.Column(scale=4):
            with gr.Row():
                chatbot = gr.Chatbot(label='得物Chatbot',height=400)
            with gr.Row():
                user_input = gr.Textbox(label='请输入问题')
            with gr.Row():
